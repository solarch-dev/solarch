/** OmniBar — AI architect chat bar embedded in BottomBar center slot. */
...
const MARKER_FOCUS_DELAY_MS = 2000;
...
function ResultPanel({
  ...
  const renderText = isInstruct ? sentences.slice(0, revealedCount).join("") : summaryText;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-border/60">
        ...
        <button type="button" onClick={onClose} aria-label="Close" className="shrink-0 inline-flex items-center justify-center w-6 h-6 rounded transition-colors text-muted-foreground hover:bg-muted hover:text-foreground">
          <X size={12} />
        </button>
      </div>
      <div className={cn("px-4 py-3 overflow-y-auto flex-1", isError ? "text-destructive" : "text-foreground")}>
        {isError ? (
          <div className="space-y-2.5">
            ...
            {onRetry && (
              <button type="button" onClick={onRetry} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded font-mono text-[12px] font-medium bg-brand-500 text-black hover:bg-brand-600 transition-colors">
                <RefreshCw size={11} />
                Try again
              </button>
            )}
          ...
          <div key={revealedCount} className={cn("text-foreground space-y-2", isInstruct && revealedCount > 0 && "animate-in fade-in duration-300")}>
            <Markdown
              components={{
      ...
      {isInstruct && !isError && (hasNext || waitingForNext || activeMarkerCount > 0) && (
        <div className="flex items-center justify-between gap-2 px-4 py-2 border-t border-border/60 bg-muted/20">
          <div className="text-[11.5px] text-muted-foreground/80 font-mono">
            {activeMarkerCount > 0 && !focusSettled ? `${activeMarkerCount} focusing references…` : waitingForNext ? "AI is typing…" : allRevealed ? "Completed" : "Ready"}
          </div>
          <button type="button" onClick={advance} disabled={!hasNext || !focusSettled} className={cn("inline-flex items-center gap-1.5 h-7 px-2.5 rounded text-[12.5px] font-medium transition-all duration-150", hasNext && focusSettled ? "bg-brand-500 text-black hover:bg-brand-600 shadow-sm hover:shadow" : "bg-muted/50 text-muted-foreground/60 cursor-not-allowed")}>
            <span>Continue</span>
