...
echo; bold "2) Clerk authentication"
dim  "   Create a free app at https://clerk.com (enable Organizations), then paste the keys."
CLERK_PK=$(read_secret "CLERK_PUBLISHABLE_KEY (pk_...)")
CLERK_SK=$(read_secret "CLERK_SECRET_KEY (sk_...)")

# ── Neo4j password ────────────────────────────────────────────────────────────
echo; bold "3) Database password (Neo4j)"
...
umask 077
{
  ...
  echo "CLERK_SECRET_KEY=$CLERK_SK"
  echo "CLERK_PUBLISHABLE_KEY=$CLERK_PK"
  echo "VITE_CLERK_PUBLISHABLE_KEY=$CLERK_PK"
  echo "GUEST_TOKEN_SECRET=$GUEST_SECRET"
...
echo
bold "✓ Wrote .env  (provider: $PROVIDER${MODEL:+, model: $MODEL}; billing disabled → unlimited)"
